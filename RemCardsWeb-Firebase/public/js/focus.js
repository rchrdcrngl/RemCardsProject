$(function() {
  ...
  firebase.auth().onAuthStateChanged(function(user) {
	  if (user) {
		  ...
		  let listRef = db.ref("/Data/" + uid).orderByChild("/tskdate").equalTo(this.date_today);
	...
	<div class="remcard">
		  <div class="card-content">
			  <span class="card-title" id="tskDesc">${tskDesc}</span>
				<p id="subjCode" class="w300">${subjCode}</p>
				<p id="tskDate" class="w200">${tskDate}</p>
				<p id="tskLvl" class="w100">${lvl}</p>
				<p hidden id="tskStat">${status}</p>
				<p hidden id="cardID">${cardID}</p>
		  </div>
		<div class="card-action">
		  <a id="updatestat">${status}</a>
		  <a id="edit" class="fa fa-edit modal-trigger" href="#card-edit" onclick="openEdit()"></a>
		  <a id="delcard" class="fa fa-trash" onclick="deleteCard()"></a>
